...
// Get best sellers - ONLY products that have been ordered, limited to exactly 15
// Using INNER JOIN ensures only products with actual orders are shown
// This query will ONLY return products that exist in order_items table
// Exclude inactive products - only show active and out_of_stock
...
if ($has_status) {
    $best_sellers_query .= " AND ((p.status = 'active' OR p.status = 'out_of_stock') AND p.status IS NOT NULL)";
}
$best_sellers_query .= "
GROUP BY p.id
HAVING COUNT(oi.order_id) > 0
ORDER BY total_quantity_sold DESC, total_orders DESC, last_ordered_date DESC
LIMIT 15";
...
// Get total count for display (only ordered products)
$total_best_sellers = $best_sellers_result ? mysqli_num_rows($best_sellers_result) : 0;
?>
...
<!-- HERO SECTION -->
<section class="best-sellers-hero">
    <video class="best-sellers-hero-video" autoplay muted loop playsinline>
        <source src="assets/images/best_sellers.mp4" type="video/mp4">
        <!-- Fallback image if video doesn't load -->
        <img src="assets/images/beads.jpg" alt="Best Sellers Background">
    </video>
    <div class="best-sellers-hero-content">
        <h1>🏆 Best Sellers</h1>
...
<div class="best-sellers-container">
    <div class="best-sellers-header">
        ...
        <div class="best-sellers-count">
            <i class="fas fa-star"></i> <?= $total_best_sellers ?> Best Selling Products
        </div>
    </div>
    
    <?php if ($best_sellers_result && mysqli_num_rows($best_sellers_result) > 0): ?>
        <div class="products-grid">
            <?php while ($product = mysqli_fetch_assoc($best_sellers_result)): 
            ...
            ?>
                <div class="product-card" onclick="window.location.href='product.php?id=<?= $product['id'] ?>'">
                    <div class="product-image-wrapper">
                        ...
                        <?php else: ?>
                            <div class="product-badge" style="background: linear-gradient(135deg, #ff6b9d, #e91e63);">
                    ...
                    <div class="product-info">
                        ...
                        <?php if ($total_orders > 0): ?>
                            <div style="font-size: 12px; color: #666; margin-bottom: 10px;">
                                <i class="fas fa-shopping-bag"></i> <?= $total_orders ?> order<?= $total_orders > 1 ? 's' : '' ?> • 
                                <i class="fas fa-box"></i> <?= $total_quantity_sold ?> sold
                            </div>
                        <?php endif; ?>
    ...
    <?php else: ?>
        <div class="empty-products">
            ...
            <h3>No Orders Yet</h3>
            <p>Best sellers will appear here once customers start placing orders. Only products that have been ordered will be shown.</p>
        </div>
    <?php endif; ?>
</div>
