...
$subcategory_name = 'Badla Dori/ Metallic Braided Cord';
...
error_log("Badla Dori - Subcategory ID: " . $subcategory_id);
error_log("Badla Dori - Query: " . $products_query);
...
if (!$products_result) {
    ...
    error_log("Badla Dori - Products found: " . $debug_count);
    if ($debug_count > 0) {
        mysqli_data_seek($products_result, 0); // Reset pointer
        $first_product = mysqli_fetch_assoc($products_result);
        error_log("Badla Dori - First product subcategory_id: " . ($first_product['subcategory_id'] ?? 'NULL') . ", Name: " . ($first_product['name'] ?? 'N/A'));
        mysqli_data_seek($products_result, 0); // Reset pointer again
    }
}

$banner_image = '../../assets/images/embroidery/badla-dori.jpg';
