...
$subcategory_name = 'Art Silk Threads';
...
error_log("Art Silk Threads - Subcategory ID: " . $subcategory_id);
error_log("Art Silk Threads - Query: " . $products_query);
error_log("Art Silk Threads - WHERE conditions: " . print_r($where_conditions, true));
...
if (!$products_result) {
    ...
    // Debug: Check what products are being returned
    $debug_count = mysqli_num_rows($products_result);
    error_log("Art Silk Threads - Products found: " . $debug_count);
    if ($debug_count > 0) {
        mysqli_data_seek($products_result, 0); // Reset pointer
        $first_product = mysqli_fetch_assoc($products_result);
        error_log("Art Silk Threads - First product subcategory_id: " . ($first_product['subcategory_id'] ?? 'NULL'));
        mysqli_data_seek($products_result, 0); // Reset pointer again
    }
}

$banner_image = '../../assets/images/embroidery/art-silk.jpg';
