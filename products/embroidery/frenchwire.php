...
// Get subcategory info for "French Wire / Dabka"
$subcategory_name = 'French Wire / Dabka';
...
// This ensures French Wire products only show on frenchwire.php
...
if (in_array('subcategory_id', $products_columns)) {
    ...
    // This ensures products are only shown when properly categorized
...
if (isset($use_join) && $use_join && in_array('subcategory_id', $products_columns)) {
    ...
    // Use INNER JOIN - this ensures products MUST have a valid subcategory_id that matches
...
// Debug: Log the query and subcategory_id for troubleshooting
error_log("French Wire - Subcategory ID: " . $subcategory_id);
error_log("French Wire - Query: " . $products_query);
error_log("French Wire - WHERE conditions: " . print_r($where_conditions, true));

// Verify subcategory_id is correct
$verify_sub = mysqli_query($conn, "SELECT id, subcategory_name FROM subcategories WHERE id = $subcategory_id LIMIT 1");
if ($verify_sub && mysqli_num_rows($verify_sub) > 0) {
    $verify_data = mysqli_fetch_assoc($verify_sub);
    error_log("French Wire - Verified subcategory: " . $verify_data['subcategory_name'] . " (ID: " . $verify_data['id'] . ")");
} else {
    error_log("French Wire - ERROR: Subcategory ID $subcategory_id not found in database!");
}

$products_result = mysqli_query($conn, $products_query);
if (!$products_result) {
    error_log("Products query failed: " . mysqli_error($conn));
    error_log("Failed query: " . $products_query);
    $products_result = false;
}

// Set banner image with cache buster to force browser reload
// Path is relative from products/embroidery/ to root: ../../assets/images/
